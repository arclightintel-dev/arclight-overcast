...
"$FF" -y \
  ...
  -framerate 24 -i "$FR/f%04d.png" \
  -filter_complex "[0:v]trim=start_frame=240:end_frame=288,setpts=PTS-STARTPTS[a];[1:v]trim=start_frame=0:end_frame=240,setpts=PTS-STARTPTS[b];[a][b]xfade=transition=fade:duration=2:offset=0,format=yuv420p[v]" \
