...
- [x] BC-01 through BC-06 closed — see §12 open questions register
...
| `vpc_cidr` | variable | PROPOSED | denied-peer-ip config | Spec §6 |
...
| `denied-peer-ip` | Yes | IP ranges, not CIDR notation | coturn docs |
...
| VPC CIDR | Terraform interpolation (`${vpc_cidr}`) | Valid CIDR | `exit 1` |
...
Transport variants: UDP implicit (default), TCP via `?transport=tcp` suffix when added. V1 uses UDP only in ICE URLs.
...
| coturn instance role | `kms:Decrypt` | **REMOVE if using default `aws/secretsmanager` key**; scope to key ARN if CMK | `kms:ViaService = secretsmanager.<region>.amazonaws.com` | Decrypt secret value | `terraform/modules/coturn/main.tf:85-97`, research findings RQ-AWS-06 |
...
**Design decision on KMS**: If the TURN secret uses the default AWS-managed key (`aws/secretsmanager`), the `kms:Decrypt` statement in the coturn IAM policy should be **removed** — it is unnecessary. If a customer-managed KMS key is used, scope the `Resource` to the specific key ARN. The current module's `Resource: *` is over-broad. Source: research findings RQ-AWS-06.
...
| VPC CIDR | Module input `var.vpc_cidr` (e.g., `10.0.0.0/16`) | DENY | `denied-peer-ip` | Allocation to VPC internal IP fails | `docs/specs/coturn-scoping.md:§1` |
...
The research spike (docs/specs/coturn-research-findings.md) resolved all RQ-OS questions at VERIFIED/LIKELY confidence from packaging source code, AWS documentation, and upstream coturn source. A disposable EC2 probe is recommended as mechanical confirmation but is not a HARD-BLOCK gate — the evidence quality from the research spike meets INV-005 requirements.
...
| `terraform plan` | 7 resources to add (SG, IAM role, policy attachment, inline policy, instance profile, EC2 instance, EIP, EIP association) | `aws_security_group.coturn`, `aws_iam_role.coturn`, `aws_instance.coturn`, `aws_eip.coturn`, `aws_eip_association.coturn` | Missing variable, AMI not found, subnet not in VPC |
...
| FM-01 | §4 Dependency ordering | Terraform interpolation eliminates race | Covered |
...
| FM-10 | §5 Rotation state machine | Single-secret with documented window | Covered |
| FM-11 | §10 Negative tests | 5 negative tests defined | Covered |
| FM-12 | §9 Abuse controls | `use-auth-secret` + short TTL + denied-peer-ip | Covered |
...
| 2 | Overcast | Step 1 or acceptance of disruption | Remove coturn module call from `staging/main.tf` | `terraform plan` shows destroy of coturn resources | Current pattern (`d441a5d`) |
| 3 | Overcast | Step 2 | `terraform apply` — destroys EC2, SG, IAM, instance profile | Apply succeeds | Terraform |
| 4 | Overcast | Step 3 | EIP: **retain** (do not destroy) — add `lifecycle { prevent_destroy = true }` or move to root module | EIP still allocated | Design decision |
| 5 | Overcast | Step 3 | Secret shell: **retain** (not managed by coturn module) | Secret still exists in SM | INV-004 |
| 6 | Overcast | Step 3 | DNS record (if created): **retain** or remove depending on reuse plan | Check Cloudflare | — |
...
| BC-01 | 3, 4, 10 | HARD-BLOCK | Package probe | Resolved |
...
- [x] O-002 cited
- [x] D-062 cited via O-002 summary; actual source marked UNRESOLVED-SOURCE (SOFT-BLOCK)
- [x] Podbay/Overcast boundary locked
- [x] Secret custody contract complete
