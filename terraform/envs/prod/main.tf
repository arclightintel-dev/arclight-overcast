...
################################################################################
# Shared account-level resources (data sources, not created here)
################################################################################
...
locals {
  ecr_repository_arns = { for k, v in data.aws_ecr_repository.repos : k => v.arn }
  ecr_repository_urls = { for k, v in data.aws_ecr_repository.repos : k => v.repository_url }
}
...
module "alb" {
  ...
  allowed_ingress_cidrs      = var.cloudflare_ipv4_cidrs
  allowed_ingress_ipv6_cidrs = var.cloudflare_ipv6_cidrs
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [local.ecr_repository_arns["arclight/dbbootstrap"]]
      },
    ]
  })
}
...
resource "aws_ecs_task_definition" "dbbootstrap" {
  ...
  container_definitions = jsonencode([
    {
      name      = "dbbootstrap"
      image     = "${local.ecr_repository_urls["arclight/dbbootstrap"]}:v1"
      essential = true
